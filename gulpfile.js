...
const tsProject = ts.createProject('./src/config/tsconfig.json');
...
gulp.task('static', gulp.series('clean', () => {
  return gulp
    .src(['src/**/*.json', 'src/**/*.mod'], { allowEmpty: true })
    .pipe(gulp.dest('dist'));
}));
...
gulp.task('watch', gulp.series('build', () => {
  return gulp.watch(['src/**/*.ts', 'src/**/*.json', 'src/**/*.mod'], gulp.series('scripts', (done) => done()));
}));
